...
describe BookyUploader do
  ...
  describe '#file_storage?' do
    context 'when file storage is used' do
      before { uploader_class.storage(:file) }
  ...
  describe '#move_to_cache' do
    it { expect(subject.move_to_cache).to eq(true) }
  end

  describe '#move_to_store' do
    it { expect(subject.move_to_store).to eq(true) }
  end
    ...
    context 'when trace is stored in Object storage' do
      ...
      context 'when file exists' do
        ...
        it 'returns http io stream' do
          expect(subject.open).to be_a(Booky::HttpIO)
        end
