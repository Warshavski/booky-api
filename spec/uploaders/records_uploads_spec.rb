...
describe RecordsUploads do
  let!(:uploader) do
    class DummyUploader < BookyUploader
    ...
    DummyUploader.new(build_stubbed(:user))
  end
  ...
  describe '#record_upload callback' do
    ...
    it 'does not create an Upload record if model is missing' do
      allow_any_instance_of(DummyUploader).to receive(:model).and_return(nil)
