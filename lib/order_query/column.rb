...
module OrderQuery
  ...
  class Column
    ...
    #
    # @param attr_name [Symbol] the name of the column,
    #   or the method providing the value to sort by.
    #
    ...
    def initialize(scope, attr_name, *vals_and_or_dir, unique: nil, nulls: false, sql: nil)
      ...
      @direction = Direction.ensure!(
        vals_and_or_dir.shift || (@order_enum ? :desc : :asc)
      )

      unless vals_and_or_dir.empty?
        raise ArgumentError, "extra arguments: #{vals_and_or_dir.map(&:inspect) * ', '}"
      end

      @unique = unique.nil? ? (name.to_s == scope.primary_key) : unique

      if @order_enum&.include?(nil)
        raise ArgumentError, '`nulls` cannot be set if a value is null' if nulls
