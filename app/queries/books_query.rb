...
class BooksQuery < ApplicationQuery
  ...
  filtering(:publisher_id) do |items, filter_value|
    items.where(publisher_id: filter_value)
  end

  filtering(:author_id) do |items, filter_value|
    items.joins(:authors).merge(Author.where(id: filter_value))
  end

  filtering(:genre_ids) do |items, filter_value|
    items.joins(:genres).merge(Genre.where(id: filter_value)).distinct
  end

  filtering(:publish_date) do |items, filter_value|
  ...
  filtering(:isbn) do |items, filter_value|
  ...
  # @param params [Hash] (optional, default: {}) filtering and sort parameters
  #
