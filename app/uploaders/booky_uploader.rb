...
# BookyUploader
#
#   Basic files uploader
#
class BookyUploader < CarrierWave::Uploader::Base
  ...
  #
  # Designed to be overridden by child uploaders that have a dynamic path segment
  # -- that is, a path that changes based on mutable attributes of its associated model
  #
  # For example, `FileUploader` builds the storage path based on the associated project model's `path_with_namespace` value,
  # which can change when the project or its containing namespace is moved or renamed.
  #
  def dynamic_segment
  ...
  def workfile_path(for_file = original_filename)
    ...
    # because calling CarrierWave.clean_cache_files! will remove any files in the cache directory.
    #
  ...
  def open_stream
    if file_storage?
      ...
      ::Booky::HttpIO.new(url, cached_size)
    end
  end
end
